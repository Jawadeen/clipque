...
class TikTokTokenStore:
    """Stores TikTok tokens in the OS credential vault (Windows Credential Manager,
    macOS Keychain, Linux Secret Service). Nothing is written to plaintext files."""
    ...
    # ── Connected open_id (persists across restarts) ──────────────────────────
...
class TikTokClient:
    ...
    Covers:
    - auth_status()   — check if a token is stored and optionally verify it against TikTok
    - upload_clip()   — full chunked video upload + publish flow
    ...
    def auth_status(self) -> TikTokAuthState:
        ...
        # Lightweight live check — fetch display name
        try:
            ...
            # Token might be expired — return connected but flag it
    ...
    def upload_clip(self, clip_row: dict, log_callback=None) -> str:
        ...
        Full TikTok Content Posting API flow:
          1. init  — register the upload with TikTok, get upload_url + publish_id
          2. chunk — upload the video file in 5 MB chunks
          3. publish — TikTok processes and publishes; poll until done

        Returns the publish_id (stored as tiktok_post_id in the queue DB).
        ...
        caption = clip_row.get("caption", "")
        file_size = video_file.stat().st_size
        chunk_count = math.ceil(file_size / CHUNK_SIZE_BYTES)
        ...
        with video_file.open("rb") as fh:
            for chunk_index in range(chunk_count):
                chunk_data   = fh.read(CHUNK_SIZE_BYTES)
                chunk_start  = chunk_index * CHUNK_SIZE_BYTES
                chunk_end    = chunk_start + len(chunk_data) - 1
                log(f"Uploading chunk {chunk_index + 1}/{chunk_count}…")
                ...
                try:
                    ...
                    raise RuntimeError(f"Chunk upload failed ({exc.code}): {exc.reason}") from exc
        ...
        for attempt in range(20):
            ...
            if status in ("FAILED", "PUBLISH_FAILED"):
                fail_code = status_resp.get("data", {}).get("fail_reason", "unknown")
    ...
    def refresh_token(self, client_key: str, client_secret: str) -> bool:
        ...
        Returns True on success, False if no refresh token is stored.
        """
        from .oauth import TIKTOK_TOKEN_URL   # local import avoids circular at module level
        open_id = self.token_store.load_open_id()
        if not open_id:
            return False
    ...
    # ── urllib.parse needed by refresh_token ──────────────────────────────────
    @staticmethod
    def _urlencode(params: dict) -> bytes:
        import urllib.parse
